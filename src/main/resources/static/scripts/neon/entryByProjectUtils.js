	...
	this.setField = function(fieldId, value) {
	    ...
	    if (element != null) {
	        switch (element.type) {
	            ...
	            case "date":     // Added date input support
	            case "datetime-local": // Added datetime-local input support
	            case "email":    // Added email input support
	            case "number":   // Added number input support
	            case "url":      // Added URL input support
	            case "tel":      // Added telephone input support
	                // console.info("Field " + fieldId + ".value = " + value);
	                element.value = value;
	                break;
	            case "radio":
	                // For radio buttons, check if the value matches and set the 'checked' property
	            ...
	            case "checkbox":
	                ...
	                element.checked = Boolean(value); // Assuming value can be coerced to boolean
	                break;
	            case "select-one":
	            case "select-multiple":
	                // For select boxes, try to set the selected option by value
	                for (let i = 0; i < element.options.length; i++) {
	                    if (element.options[i].value == value) {
	                        element.selectedIndex = i;
	                        return; // Exit after setting the selection
	                    }
	                }
	                // If the value doesn't match, fall back to setting by index if value is a number
	                if (!isNaN(parseInt(value))) {
	                    setInputSelectionIndex(element, value);
	                }
	                break;
	            default:
	                // Fallback for other types, try setting by index if value is a number
	                if (!isNaN(parseInt(value))) {
	                    setInputSelectionIndex(element, value);
	                } else {
	                    // console.info("Field type '" + element.type + "' for " + fieldId + " not explicitly handled. Attempting setInputSelectionIndex.");
	                    setInputSelectionIndex(element, value);
	                }
	                break;
	        }
	    } else {
	        // console.info("ObservationHistory.setField: " + fieldId + " does not exist."); // firebugs only
	    }
	};
...
function SampleLoader() {
	...
	this.processLoadSuccess = function /* void */(xhr) {
		...
		var sampleType = this.getResponseProperty(response, "sampleType");
		
		
		
		this.setFields(samplePK, labNo, receivedDate, interviewDate, centerName, centerCode, sampleType);
	...
	this.setFields = function /* void */(samplePK, labNo, receivedDate, interviewDate, centerName, centerCode, sampleType) {
		$("samplePK").value = blankOrValue(samplePK);
		labNo = blankOrValue(labNo);
		this.setFieldInAllStudies("labNo", labNo);
		this.setFieldInAllStudies("labNoForDisplay", labNo.substring(4));
		...
		this.setFieldInAllStudies("centerName", blankOrValue(centerName));
		this.setFieldInAllStudies("centerCode", blankOrValue(centerCode));
		if(sampleType === 'EDTA Tube'){
			this.setFieldInAllStudies("edtaTubeTaken", true);
		}else if(sampleType === 'Dry Tube'){
			this.setFieldInAllStudies("dbsvlTaken", true);
		}else if(sampleType === 'PSC'){
			this.setFieldInAllStudies("pscvlTaken", true);
		}
	};
	...
	this.checkGender = function(blanksAllowed) {
		...
		if (this.idPre === 'vl.' || this.idPre === 'rt.') {// do it only for viral load form
			if (selectedValue === 'F') {
				...
				$(this.idPre + "vlPregnancy").value = "";
				$(this.idPre + "vlSuckle").value = "";

...
public class SampleDAOImpl extends BaseDAOImpl<Sample, String> implements SampleDAO {
	...
	public List<Sample> getSampleByPatientAndTestAndCollectionDate(String patientIdentifier, Test test,
		...
		String hql = " FROM Sample s " + " WHERE s.id IN (" + " SELECT si.sample.id " + " FROM SampleItem si "
				+ " WHERE ((cast(si.collectionDate as date) BETWEEN  :startDate AND :endDate) "
				+ " OR s.collectionDate BETWEEN :startDate AND :endDate )) " + " AND s.id IN ( "
				+ " SELECT sh.sampleId " + " FROM SampleHuman sh, Patient p  " + " WHERE sh.patientId = p.id AND "
				+ " (lower(p.nationalId) = lower(:patientIdentifier) "
				+ " OR lower(p.externalId) = lower(:patientIdentifier))" + " )" + " AND s.id IN ("
				+ "    SELECT a.sampleItem.sample.id " + "    FROM Analysis a " + "    WHERE a.test.id = :testId "
	...
	public Sample getById(String id) {
		String sql = "from Sample s where s.id = :id";
